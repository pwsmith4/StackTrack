...
function LocationTypeAdministration({ types, session, refresh, canManage, onNotice, onError }: { types: LocationType[]; session: AdminSession | null; refresh: () => Promise<void>; canManage: boolean; onNotice: (message: string) => void; onError: (message: string) => void }) {
  ...
  return <details className="location-type-admin location-admin-disclosure">
    <summary className="location-admin-disclosure__summary"><span className="location-admin-disclosure__summary-copy"><span className="eyebrow">Location types</span><strong>Set the names and icons used across the directory.</strong><small>{types.length} type{types.length === 1 ? "" : "s"} available. Open this section to add a type or edit an existing type’s name and icon.</small></span><ChevronDown className="location-admin-disclosure__chevron" size={17} /></summary>
