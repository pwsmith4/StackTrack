...
import { parseContainerCsv, validateContainerRows, type ContainerCsvRecord, type ContainerImportValidationError } from "./container-import";
...
function LocationTypeIcon({ location, size = 18 }: { location: Location; size?: number }) {
  const iconByKey: Record<string, typeof Store> = {
    store: Store,
    warehouse: Warehouse,
  ...
  const Icon = iconByKey[location.iconKey ?? ""] ?? (location.type === "warehouse" ? Warehouse : location.type === "donation_express" ? HandHeart : location.type === "in_transit" ? Truck : Store);
...
function humanizeDetailValue(key: string, value: unknown, data?: OperationsData): string {
  ...
  if (typeof value === "number" || typeof value === "string") {
    ...
    if (data && (key === "assigned_location_id" || key === "assignedLocationId" || key === "locationId" || key === "sourceLocationId" || key === "destinationLocationId")) {
  ...
  if (isRecord(value)) {
    return Object.entries(value)
      .filter(([nestedKey]) => nestedKey !== "destinationLocationId")
...
function detailChangeRows(details: Record<string, unknown>, data?: OperationsData): [string, ReactNode][] {
  ...
  return keys
    // A receiving site is not known at departure.  Older clients could have
    // persisted a planned destination, but it is not evidence and should not
    // be presented as though an administrator confirmed it.
...
function ReadableDetails({ details, data, emptyLabel = "No additional details were recorded." }: { details: Record<string, unknown>; data?: OperationsData; emptyLabel?: string }) {
  ...
  const fields = Object.entries(details)
    .filter(([key]) => key !== "before" && key !== "after" && key !== "destinationLocationId")
      ...
      <aside className={`sidebar ${menuOpen ? "sidebar--open" : ""}`}>
        ...
        <div className="sidebar__bottom">
          ...
          <button className="user-card" onClick={() => session ? setDetail({
            eyebrow: "Signed-in profile", title: session.principal.displayName, icon: <UserRound size={18} />, status: { label: roleLabel(session.principal.role), tone: session.principal.role === "organization_owner" ? "blue" : "good" }, summary: "Your Goodwill operations account and access details.", recordId: session.principal.userId, recordIdLabel: "Administrator ID",
            body: <><p className="detail-lead">This profile controls access to the Goodwill operations console. Administrative changes are recorded under this account.</p><h3 className="detail-section-title">Account access</h3><DetailFacts items={[["Username", session.principal.username], ["Role", roleLabel(session.principal.role)], ["Scope", scopeLabel(session.principal)]]}/><h3 className="detail-section-title">Session status</h3><div className="profile-security-card"><CheckCircle2 size={18}/><div><strong>Secure session active</strong><span>Automatically expires {new Date(session.expiresAt).toLocaleString()}</span><small>The API has verified this sign-in. Signing out ends the session immediately.</small></div></div><div className="profile-signout"><span className="eyebrow">Account actions</span><strong>Sign out of StackTrack</strong><p>End this browser session when you are finished. You can sign back in anytime.</p><button className="secondary profile-signout__button" onClick={() => void signOut()}>Sign out</button></div></>
          }) : setSignInOpen(true)}>
...
function buildInventorySnapshotRecords(data: OperationsData): InventorySnapshotRecord[] {
  ...
  return data.fixtures.containers.map<InventorySnapshotRecord>((container) => {
    ...
    return {
      ...
      locationType: locationKey === unknownKey ? "Needs assignment" : location ? locationTypeLabel(location) : "Needs assignment",
...
function WarehouseInventoryOverview({ data, setPage, openLocation }: { data: OperationsData; setPage: (page: Page) => void; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void }) {
  ...
  return <section className="panel warehouse-inventory-panel warehouse-inventory-panel--compact">
...
function WarehouseForecastPanel({ data, warehouses, warehouseRecords, goodsColumns, openLocation }: { data: OperationsData; warehouses: Location[]; warehouseRecords: InventorySnapshotRecord[]; goodsColumns: string[]; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void }) {
  ...
  const eventAdjustmentFor = (warehouseId: string) => activePlanningEvents
    ...
    .filter((event) => event.goodsType === "all" || event.goodsType === goodsScope)
  ...
  return <section className="panel warehouse-forecast-panel" id="warehouse-capacity" aria-labelledby="warehouse-capacity-title">
    <div className="warehouse-forecast__header"><div><span className="eyebrow">Planning forecast</span><h2 id="warehouse-capacity-title">Warehouse capacity outlook</h2><p>Estimate upcoming receipts and recommended on-hand containers using accepted scan history, safety stock, growth, and planned events.</p></div><div className="warehouse-forecast__actions"><button type="button" className="secondary" onClick={() => setPlannerOpen((value) => !value)}><CalendarDays size={15} /> {plannerOpen ? "Close event planner" : "Plan a holiday or event"}</button><button type="button" className="secondary" onClick={exportForecast} disabled={!rows.length}><Download size={15} /> Export forecast</button></div></div>
    <div className="warehouse-forecast__assumption-note"><Target size={16} /><span><strong>This is a planning estimate, not an official inventory correction.</strong><small>Planning inputs stay separate from scanner history and affect recommendations only. Accepted observations remain the source of truth.</small></span><Pill tone={forecastConfidence === "High" ? "good" : forecastConfidence === "Medium" ? "blue" : "warn"}>{forecastConfidence} confidence</Pill></div>
    ...
    <WarehouseForecastSavedEvents planningEvents={planningEvents} warehouses={warehouses} onRemove={(id) => setPlanningEvents((current) => current.filter((item) => item.id !== id))} />
  </section>;
}

function WarehouseForecastSavedEvents({ planningEvents, warehouses, onRemove }: { planningEvents: WarehousePlanningEvent[]; warehouses: Location[]; onRemove: (id: string) => void }) {
  if (!planningEvents.length) return null;
  return <section className="warehouse-forecast__saved-events"><div><span className="eyebrow">Scenario adjustments</span><h3>{planningEvents.length} saved forecast input{planningEvents.length === 1 ? "" : "s"}</h3><p>These dated assumptions affect the forecast only. They do not change scanner history or official counts.</p></div><div className="warehouse-forecast__event-list">{planningEvents.map((event) => <article key={event.id}><span className="warehouse-forecast__event-icon"><CalendarDays size={15} /></span><span><strong>{event.name}</strong><small>{event.date} · {event.durationDays} days · {event.upliftPercent >= 0 ? "+" : ""}{event.upliftPercent}% · {event.warehouseId === "all" ? "All warehouses" : warehouses.find((warehouse) => warehouse.locationId === event.warehouseId)?.name ?? "Selected warehouse"}{event.goodsType === "all" ? "" : ` · ${event.goodsType}`}</small></span><button type="button" onClick={() => onRemove(event.id)} aria-label={`Remove ${event.name}`}><Trash2 size={14} /></button></article>)}</div></section>;
}
...
interface StoreOutlookGroup {
  store: Location;
  rows: StoreOutlookRow[];
}

function StoreCoverageQueue({ groups, totalGroups, totalRows, pageSize, page, pageCount, horizonDays, onPageSizeChange, onPageChange, onConfigure }: { groups: StoreOutlookGroup[]; totalGroups: number; totalRows: number; pageSize: 10 | 20 | 50 | "all"; page: number; pageCount: number; horizonDays: number; onPageSizeChange: (size: 10 | 20 | 50 | "all") => void; onPageChange: (page: number) => void; onConfigure: (row: StoreOutlookRow) => void }) {
  return <section id="store-coverage" className="store-outlook__queue" aria-labelledby="store-coverage-queue-title">
    <div className="store-outlook__queue-heading"><div><span className="eyebrow">Store coverage queue</span><h3 id="store-coverage-queue-title">Locations that need planning attention</h3><p>Locations are grouped together so every goods category and container type can be reviewed without mixing separate stores.</p></div><div className="store-outlook__queue-controls"><label><span>Locations per page</span><select value={pageSize} onChange={(event) => onPageSizeChange(event.target.value === "all" ? "all" : Number(event.target.value) as 10 | 20 | 50)}><option value={10}>10 locations</option><option value={20}>20 locations</option><option value={50}>50 locations</option><option value="all">All locations</option></select></label><span className="store-outlook__queue-count">Showing {groups.length} of {totalGroups} locations · {totalRows} item rows</span></div></div>
    <div className="store-outlook__group-table-wrap table-wrap"><table className="store-outlook__group-table"><thead><tr><th>Item category</th><th>Container type</th><th>Current on hand</th><th>Expected movement</th><th>Recommended on hand</th><th>Target range</th><th>Planning gap</th><th>Seasonal input</th><th></th></tr></thead><tbody>{groups.length ? groups.map((group) => <Fragment key={group.store.locationId}><tr className="store-outlook__group-row"><th colSpan={9}><span><Store size={14} /><strong>{group.store.name}</strong><small>{group.rows.length} item type{group.rows.length === 1 ? "" : "s"} in this view · {group.rows.filter((row) => row.gap > 0).length} planning signal{group.rows.filter((row) => row.gap > 0).length === 1 ? "" : "s"}</small></span><b>{group.rows.reduce((sum, row) => sum + row.current, 0)} on hand · {group.rows.reduce((sum, row) => sum + Math.max(0, row.gap), 0)} to plan</b></th></tr>{group.rows.map((row) => <tr key={storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)}><th><strong>{row.goodsType}</strong></th><td>{containerTypeLabel(row.containerType)}</td><td>{row.current}<small>{Math.round(row.weeklyBaseline * 10) / 10}/week baseline</small></td><td>{row.expected}<small>Next {horizonDays} days</small></td><td><strong>{row.recommended}</strong><small>Target + safety buffer</small></td><td>{row.target.minimumOnHand}–{row.target.maximumOnHand}</td><td className={row.gap > 0 ? "store-outlook__gap--attention" : "store-outlook__gap--ready"}>{row.gap > 0 ? `+${row.gap}` : "On target"}<small>{row.gap > 0 ? "Plan delivery" : row.current > row.target.maximumOnHand ? "Review excess" : "No action"}</small></td><td>{row.holidayUplift ? <span className="store-outlook__holiday-chip">{row.holidayUplift > 0 ? "+" : ""}{row.holidayUplift}%<small>{row.holidayNames[0]}</small></span> : <span className="store-outlook__no-holiday">No adjustment</span>}</td><td><button type="button" className="secondary" onClick={() => onConfigure(row)}>Edit target</button></td></tr>)}</Fragment>) : <tr><td colSpan={9}><EmptyState>No store rows match these filters.</EmptyState></td></tr>}</tbody></table></div>
    {pageCount > 1 && <div className="store-outlook__queue-pagination"><button type="button" className="secondary" disabled={page === 0} title={page === 0 ? "Already showing the first page" : "Show the previous locations"} aria-label={page === 0 ? "Previous locations unavailable: first page" : "Show previous locations"} onClick={() => onPageChange(Math.max(0, page - 1))}>Previous locations</button><span>Page {page + 1} of {pageCount}</span><button type="button" className="secondary" disabled={page >= pageCount - 1} title={page >= pageCount - 1 ? "Already showing the last page" : "Show the next locations"} aria-label={page >= pageCount - 1 ? "Next locations unavailable: last page" : "Show next locations"} onClick={() => onPageChange(Math.min(pageCount - 1, page + 1))}>Next locations</button></div>}
  </section>;
}

function StoreCoverageQueueCollapsible({ groups, totalGroups, totalRows, pageSize, page, pageCount, horizonDays, storeSearch, onStoreSearchChange, onPageSizeChange, onPageChange, onConfigure, onOpenLocation, attentionOnly, onAttentionOnlyChange }: {
  ...
  return <section id="store-coverage" className="store-outlook__queue" aria-labelledby="store-coverage-queue-title">
    <div className="store-outlook__queue-heading">
      ...
      <div className="store-outlook__queue-controls">
        ...
        <label className="store-outlook__attention-toggle"><input type="checkbox" checked={attentionOnly} onChange={(event) => onAttentionOnlyChange(event.target.checked)} /><span>Only show locations needing a plan</span></label>
    ...
    <div className="store-outlook__group-table-wrap table-wrap"><table className="store-outlook__group-table"><thead><tr><th>Item category</th><th>Container type</th><th>Current on hand</th><th>Expected movement</th><th>Recommended on hand</th><th>Target range</th><th>Planning gap</th><th>Seasonal input</th><th></th></tr></thead><tbody>{groups.length ? groups.map((group) => {
      ...
      return <Fragment key={group.store.locationId}>
        ...
        {expanded && group.rows.map((row) => <tr key={storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)}><th><strong>{row.goodsType}</strong></th><td>{containerTypeLabel(row.containerType)}</td><td>{row.current}<small>{Math.round(row.weeklyBaseline * 10) / 10}/week baseline</small></td><td>{row.expected}<small>Next {horizonDays} days</small></td><td><strong>{row.recommended}</strong><small>Target + safety buffer</small></td><td>{row.target.minimumOnHand}–{row.target.maximumOnHand}</td><td className={row.gap > 0 ? "store-outlook__gap--attention" : "store-outlook__gap--ready"}>{row.gap > 0 ? `+${row.gap}` : "On target"}<small>{row.gap > 0 ? "Plan delivery" : row.current > row.target.maximumOnHand ? "Review excess" : "No action"}</small></td><td>{row.holidayUplift ? <span className="store-outlook__holiday-chip">{row.holidayUplift > 0 ? "+" : ""}{row.holidayUplift}%<small>{row.holidayNames[0]}</small></span> : <span className="store-outlook__no-holiday">No adjustment</span>}</td><td><button type="button" className="secondary" onClick={() => onConfigure(row)}>Edit target</button></td></tr>)}
      </Fragment>;
...
function StoreHolidayPlanner({ holidays, stores, goodsColumns, editorOpen, setEditorOpen, holidayName, setHolidayName, holidayDate, setHolidayDate, holidayDuration, setHolidayDuration, holidayUplift, setHolidayUplift, holidayLocation, setHolidayLocation, holidayGoods, setHolidayGoods, onAdd, onUpdate, onRemove }: { holidays: StoreHolidayAdjustment[]; stores: Location[]; goodsColumns: string[]; editorOpen: boolean; setEditorOpen: (open: boolean) => void; holidayName: string; setHolidayName: (value: string) => void; holidayDate: string; setHolidayDate: (value: string) => void; holidayDuration: string; setHolidayDuration: (value: string) => void; holidayUplift: string; setHolidayUplift: (value: string) => void; holidayLocation: string; setHolidayLocation: (value: string) => void; holidayGoods: string; setHolidayGoods: (value: string) => void; onAdd: (event: React.FormEvent) => void; onUpdate: (id: string, patch: Partial<StoreHolidayAdjustment>) => void; onRemove: (id: string) => void }) {
  return <section id="seasonal-planning" className="store-outlook__holiday-section store-outlook__holiday-section--new"><div className="store-outlook__section-heading"><div><span className="eyebrow">Seasonal planning settings</span><h3>Adjust the calendar used by the forecast</h3><p>Keep holiday and local-event assumptions separate from everyday filters. Enable, disable, or tune an adjustment without changing scan history.</p></div><div className="store-outlook__holiday-heading-actions"><span className="store-outlook__holiday-count">{holidays.filter((holiday) => holiday.enabled).length} active</span><button type="button" className="secondary store-outlook__holiday-toggle" onClick={() => setEditorOpen(!editorOpen)}><Plus size={14} /> {editorOpen ? "Close editor" : "Add seasonal adjustment"}</button></div></div>{editorOpen && <div className="store-outlook__holiday-create"><span className="eyebrow">New planning assumption</span><strong>Choose when the forecast should move differently from normal history</strong><p>Use a positive value for higher movement and a negative value for a slower period. Apply it to every store or a specific location and goods category.</p><form className="store-outlook__holiday-form" onSubmit={onAdd}><label><span>Holiday or event</span><input required value={holidayName} onChange={(event) => setHolidayName(event.target.value)} placeholder="Community donation drive" /></label><label><span>Start date</span><input required type="date" value={holidayDate} onChange={(event) => setHolidayDate(event.target.value)} /></label><label><span>Duration (days)</span><input type="number" min="1" max="90" value={holidayDuration} onChange={(event) => setHolidayDuration(event.target.value)} /></label><label><span>Expected movement change %</span><input type="number" min="-90" max="300" value={holidayUplift} onChange={(event) => setHolidayUplift(event.target.value)} /></label><label><span>Store scope</span><select value={holidayLocation} onChange={(event) => setHolidayLocation(event.target.value)}><option value="all">All stores</option>{stores.map((store) => <option value={store.locationId} key={store.locationId}>{store.name}</option>)}</select></label><label><span>Goods scope</span><select value={holidayGoods} onChange={(event) => setHolidayGoods(event.target.value)}><option value="all">All categories</option>{goodsColumns.map((goodsType) => <option value={goodsType} key={goodsType}>{goodsType}</option>)}</select></label><button type="submit" className="primary"><Plus size={15} /> Save adjustment</button></form></div>}<div className="store-outlook__holiday-list">{holidays.map((holiday) => <article key={holiday.id} className={holiday.enabled ? "" : "store-outlook__holiday--disabled"}><span className="store-outlook__holiday-icon"><CalendarDays size={15} /></span><div><strong>{holiday.name}</strong><small>{holiday.date} · {holiday.durationDays} day{holiday.durationDays === 1 ? "" : "s"} · {holiday.locationId === "all" ? "All stores" : stores.find((store) => store.locationId === holiday.locationId)?.name ?? "Selected store"} · {holiday.goodsType === "all" ? "All categories" : holiday.goodsType}</small></div><label className="store-outlook__holiday-number"><span>Movement change %</span><input type="number" min="-90" max="300" value={holiday.upliftPercent} onChange={(event) => onUpdate(holiday.id, { upliftPercent: Math.max(-90, Math.min(300, Number(event.target.value) || 0)) })} /></label><button type="button" className="secondary" onClick={() => onUpdate(holiday.id, { enabled: !holiday.enabled })}>{holiday.enabled ? "Disable" : "Enable"}</button><button type="button" className="store-outlook__remove" onClick={() => onRemove(holiday.id)} aria-label={`Remove ${holiday.name}`}><Trash2 size={14} /></button></article>)}</div></section>;
}

function LocationTargetWorkspace({ stores, rows, targets, editorStore, setEditorStore, onSaveValues, onResetValues, editRequestKey, onEditRequestHandled }: { stores: Location[]; rows: StoreOutlookRow[]; targets: Record<string, StoreOutlookTarget>; editorStore: string; setEditorStore: (value: string) => void; onSaveValues: (locationId: string, goodsType: string, containerType: ServiceContainerType, minimum: number, maximum: number) => void; onResetValues: (locationId: string, goodsType: string, containerType: ServiceContainerType) => void; editRequestKey: string | null; onEditRequestHandled: () => void }) {
  const [attentionOnly, setAttentionOnly] = useState(false);
  const [editingKey, setEditingKey] = useState<string | null>(null);
  const [draftMinimum, setDraftMinimum] = useState("");
  const [draftMaximum, setDraftMaximum] = useState("");
  const [rangeError, setRangeError] = useState("");
  const selectedStore = stores.find((store) => store.locationId === editorStore) ?? stores[0];
  const requestedRow = editRequestKey ? rows.find((row) => storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType) === editRequestKey) : null;

  useEffect(() => {
    if (!editRequestKey || !requestedRow || requestedRow.store.locationId !== selectedStore?.locationId) return;
    setEditingKey(editRequestKey);
    setDraftMinimum(String(requestedRow.target.minimumOnHand));
    setDraftMaximum(String(requestedRow.target.maximumOnHand));
    setRangeError("");
    onEditRequestHandled();
  }, [editRequestKey, selectedStore?.locationId]);

  if (!selectedStore) return <section id="store-targets" className="store-outlook__target-editor store-outlook__target-editor--new"><EmptyState>No store locations are available for target setup.</EmptyState></section>;
  const locationRows = rows.filter((row) => row.store.locationId === selectedStore.locationId);
  const detailRows = attentionOnly ? locationRows.filter((row) => row.current < row.target.minimumOnHand || row.current > row.target.maximumOnHand) : locationRows;
  const belowMinimum = locationRows.filter((row) => row.current < row.target.minimumOnHand).length;
  const aboveMaximum = locationRows.filter((row) => row.current > row.target.maximumOnHand).length;
  const configured = locationRows.filter((row) => Boolean(targets[storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)])).length;
  const historyCoverage = locationRows.length ? Math.max(...locationRows.map((row) => row.historyWeeks)) : 0;
  const historyCoverageLabel = historyCoverage >= 8 ? "8 of 8 weeks with history" : historyCoverage ? `${historyCoverage} of 8 weeks with history` : "No load history yet";
  const statusFor = (row: StoreOutlookRow): { label: string; tone: PillTone; detail: string } => {
    if (row.current < row.target.minimumOnHand) return { label: "Below minimum", tone: "warn", detail: `${row.target.minimumOnHand - row.current} below the minimum` };
    if (row.current > row.target.maximumOnHand) return { label: "Above maximum", tone: "blue", detail: `${row.current - row.target.maximumOnHand} above the maximum` };
    return { label: "Within range", tone: "good", detail: `${row.target.maximumOnHand - row.current} before the maximum` };
  };
  const startEditing = (row: StoreOutlookRow) => {
    setEditingKey(storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType));
    setDraftMinimum(String(row.target.minimumOnHand));
    setDraftMaximum(String(row.target.maximumOnHand));
    setRangeError("");
  };
  const cancelEditing = () => { setEditingKey(null); setRangeError(""); };
  const saveEditing = (row: StoreOutlookRow) => {
    const minimum = Number(draftMinimum);
    const maximum = Number(draftMaximum);
    if (!draftMinimum.trim() || !draftMaximum.trim() || !Number.isInteger(minimum) || !Number.isInteger(maximum) || minimum < 0 || maximum < minimum || maximum > 999) {
      setRangeError("Enter whole numbers from 0 to 999, with the maximum at least as high as the minimum.");
      return;
    }
    onSaveValues(row.store.locationId, row.goodsType, row.containerType, minimum, maximum);
    cancelEditing();
  };
  const resetEditing = (row: StoreOutlookRow) => { onResetValues(row.store.locationId, row.goodsType, row.containerType); cancelEditing(); };
  return <section id="store-targets" className="store-outlook__target-editor store-outlook__target-editor--new" aria-labelledby="location-target-setup-title">
    <div className="store-outlook__target-header"><div><span className="eyebrow">Location target setup</span><h3 id="location-target-setup-title">Choose a location, then tune its item targets</h3><p>Use the selector to focus one store. Edit minimum and maximum values directly in the row so the effect of each change stays visible.</p></div><label className="store-outlook__target-location"><span>Location</span><select value={selectedStore.locationId} onChange={(event) => { setEditorStore(event.target.value); cancelEditing(); }}>{stores.map((store) => <option key={store.locationId} value={store.locationId}>{store.name}</option>)}</select></label></div>
    <div className="store-outlook__target-summary-grid"><div><span><MapPin size={14} />Selected location</span><strong>{selectedStore.name}</strong><small>Store operating targets</small></div><div><span><ContainerIcon size={14} />Item types</span><strong>{locationRows.length}</strong><small>Goods and container combinations</small></div><div className={belowMinimum ? "store-outlook__target-summary-card--attention" : ""}><span><AlertTriangle size={14} />Below minimum</span><strong>{belowMinimum}</strong><small>{belowMinimum || aboveMaximum ? `${belowMinimum} below minimum · ${aboveMaximum} above maximum` : "All combinations in range"}</small></div><div><span><CheckCircle2 size={14} />Custom targets</span><strong>{configured}/{locationRows.length || 0}</strong><small>{configured ? "Saved for this location" : "Using default ranges"}</small></div></div>
    <div className="store-outlook__target-details"><div className="store-outlook__target-details-heading"><div><span className="eyebrow">Item targets at {selectedStore.name}</span><strong>Review and edit the range in place</strong><p>Every row is one goods category and container type. Values are planning assumptions only; they never change scan history. <span className="store-outlook__coverage-inline">{historyCoverageLabel}</span></p></div><label className="store-outlook__target-check"><input type="checkbox" checked={attentionOnly} onChange={(event) => setAttentionOnly(event.target.checked)} /> <span>Show planning signals only</span></label></div>{rangeError && <p className="store-outlook__target-error" role="alert">{rangeError}</p>}<div className="store-outlook__target-table-wrap table-wrap"><table className="store-outlook__target-table"><thead><tr><th>Goods category</th><th>Container</th><th>On hand</th><th>Target range</th><th>Status</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{detailRows.length ? detailRows.map((row) => { const status = statusFor(row); const key = storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType); const isEditing = editingKey === key; return <tr className={isEditing ? "store-outlook__target-row--selected" : ""} key={key}><th><strong>{row.goodsType}</strong><small>{row.holidayUplift ? `${row.holidayUplift > 0 ? "+" : ""}${row.holidayUplift}% seasonal adjustment` : "No seasonal adjustment"}</small></th><td>{containerTypeLabel(row.containerType)}</td><td><strong>{row.current}</strong><small>{Math.round(row.weeklyBaseline * 10) / 10}/week · {row.historyWeeks}/8 weeks</small></td><td className="store-outlook__target-range-cell">{isEditing ? <div className="store-outlook__inline-range"><input type="number" min="0" max="999" value={draftMinimum} onChange={(event) => setDraftMinimum(event.target.value)} aria-label={`${row.goodsType} ${containerTypeLabel(row.containerType)} minimum on hand`} /><span>to</span><input type="number" min="0" max="999" value={draftMaximum} onChange={(event) => setDraftMaximum(event.target.value)} aria-label={`${row.goodsType} ${containerTypeLabel(row.containerType)} maximum on hand`} /></div> : <><strong>{row.target.minimumOnHand}–{row.target.maximumOnHand}</strong><small>{targets[key] ? "Custom range" : "Default range"}</small></>}</td><td><Pill tone={status.tone}>{status.label}</Pill><small>{status.detail}</small></td><td className="store-outlook__target-action-cell">{isEditing ? <div className="store-outlook__inline-actions"><button type="button" className="primary" onClick={() => saveEditing(row)}>Save</button><button type="button" className="secondary" onClick={cancelEditing}>Cancel</button><button type="button" className="store-outlook__inline-reset" onClick={() => resetEditing(row)}>Use default</button></div> : <button type="button" className="secondary" onClick={() => startEditing(row)}>Edit target</button>}</td></tr>; }) : <tr><td colSpan={6}><EmptyState>No item types match the selected view.</EmptyState></td></tr>}</tbody></table></div></div>
  </section>;
}
...
const storeOutlookQueuePageSizeKey = "stacktrack.warehouse.store-outlook.queue-page-size";
...
function StoreCapacityOutlook({ data, records, goodsColumns, openLocation }: { data: OperationsData; records: InventorySnapshotRecord[]; goodsColumns: string[]; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void }) {
  ...
  // Kept for the legacy form markup during the transition; the visible editor
  // is now the inline row editor in LocationTargetWorkspace.
  const [editorMinimum, setEditorMinimum] = useState("6");
  const [editorMaximum, setEditorMaximum] = useState("18");
  const [targetEditRequest, setTargetEditRequest] = useState<string | null>(null);
  ...
  const [storeQueuePageSize, setStoreQueuePageSize] = useState<10 | 20 | 50 | "all">(() => {
    const saved = readWarehouseForecastStorage<unknown>(storeOutlookQueuePageSizeKey, 10);
  ...
  const [storeQueueAttentionOnly, setStoreQueueAttentionOnly] = useState(false);
  ...
  useEffect(() => { try { window.localStorage.setItem(storeOutlookQueuePageSizeKey, String(storeQueuePageSize)); } catch { /* local preference remains optional */ } }, [storeQueuePageSize]);
  ...
  const rows: StoreOutlookRow[] = stores.flatMap((store) => goodsColumns.flatMap((goodsType) => containerOptions.map((containerType) => {
    ...
    const historyEvents = data.events.filter((event) => {
      const eventAt = Date.parse(event.eventAt);
      return event.locationId === store.locationId && event.eventType === "load_assigned" && eventGoodsType(event) === goodsType && eventContainerType(event) === containerType && Number.isFinite(eventAt) && eventAt > addDays(referenceNow, -historyWeeks * 7).getTime() && eventAt <= referenceNow.getTime();
    });
    const historyWeekKeys = new Set(historyEvents.map((event) => Math.max(0, Math.min(historyWeeks - 1, Math.floor((referenceNow.getTime() - Date.parse(event.eventAt)) / (7 * 24 * 60 * 60 * 1_000))))));
    ...
    return { store, goodsType, containerType, current, weeklyBaseline, historyWeeks: historyWeekKeys.size, expected, recommended, gap: recommended - current, target, holidayUplift, holidayNames: holidayEvents.map((event) => event.name) };
  })));
  ...
  const storeGroups = Array.from(visibleRows.reduce((groups, row) => {
    const existing = groups.get(row.store.locationId);
  ...
  const searchedStoreGroups = normalizedStoreQueueSearch
    ...
    : storeGroups;
  const filteredStoreGroups = storeQueueAttentionOnly
    ? searchedStoreGroups.filter((group) => group.rows.some((row) => row.gap > 0 || row.current > row.target.maximumOnHand))
    : searchedStoreGroups;
  const storePageCount = storeQueuePageSize === "all" ? 1 : Math.max(1, Math.ceil(filteredStoreGroups.length / storeQueuePageSize));
  const safeStoreQueuePage = Math.min(storeQueuePage, storePageCount - 1);
  const visibleStoreGroups = storeQueuePageSize === "all" ? filteredStoreGroups : filteredStoreGroups.slice(safeStoreQueuePage * storeQueuePageSize, (safeStoreQueuePage + 1) * storeQueuePageSize);
  const visibleStoreRows = visibleStoreGroups.flatMap((group) => group.rows);
  ...
  const saveTargetValues = (locationId: string, goodsType: string, containerType: ServiceContainerType, minimumValue: number, maximumValue: number) => {
    const minimum = Math.max(0, Math.min(999, Math.round(minimumValue)));
    const maximum = Math.max(minimum, Math.min(999, Math.round(maximumValue)));
    setTargets((current) => ({ ...current, [storeOutlookTargetKey(locationId, goodsType, containerType)]: { locationId, goodsType, containerType, minimumOnHand: minimum, maximumOnHand: maximum } }));
  };
  const resetTargetValues = (locationId: string, goodsType: string, containerType: ServiceContainerType) => setTargets((current) => {
    const next = { ...current };
    delete next[storeOutlookTargetKey(locationId, goodsType, containerType)];
    return next;
  });
  const saveTarget = (event: React.FormEvent) => {
    ...
    saveTargetValues(editorStore, editorGoods, editorContainer, Number(editorMinimum) || 0, Number(editorMaximum) || Number(editorMinimum) || 0);
  };
  const resetTarget = () => resetTargetValues(editorStore, editorGoods, editorContainer);
  ...
  return <section id="store-capacity" className="panel store-outlook-panel">
    <LocationTargetWorkspace stores={stores} rows={rows} targets={targets} editorStore={editorStore} setEditorStore={setEditorStore} onSaveValues={saveTargetValues} onResetValues={resetTargetValues} editRequestKey={targetEditRequest} onEditRequestHandled={() => setTargetEditRequest(null)} />
    <StoreCoverageQueueCollapsible groups={visibleStoreGroups} totalGroups={filteredStoreGroups.length} totalRows={visibleStoreRows.length} pageSize={storeQueuePageSize} page={safeStoreQueuePage} pageCount={storePageCount} horizonDays={settings.horizonDays} storeSearch={storeQueueSearch} onStoreSearchChange={(value) => { setStoreQueueSearch(value); setStoreQueuePage(0); }} onPageSizeChange={(size) => { setStoreQueuePageSize(size); setStoreQueuePage(0); }} onPageChange={setStoreQueuePage} attentionOnly={storeQueueAttentionOnly} onAttentionOnlyChange={(value) => { setStoreQueueAttentionOnly(value); setStoreQueuePage(0); }} onConfigure={(row) => { setEditorStore(row.store.locationId); setTargetEditRequest(storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)); document.getElementById("store-targets")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} onOpenLocation={(locationId) => openLocation(locationId)} />
    ...
    <div id="forecast-method" className="store-outlook__logic"><div><span className="eyebrow">How StackTrack calculates this</span><h3>Every recommendation is explainable</h3><p>The forecast uses accepted scanner observations as a transparent starting point. Historical inventory snapshots can be added later to improve seasonality and route-aware planning without changing the user workflow.</p></div><div className="store-outlook__logic-grid"><div><b>1</b><span><strong>Historical baseline</strong><small>Count load-assignment scans for the store, goods category, and container type over the last eight seven-day periods, then average them into weekly movement.</small></span></div><div><b>2</b><span><strong>Expected movement</strong><small>Weekly baseline × selected horizon ÷ 7, adjusted by the growth setting and any active holiday inputs for that store.</small></span></div><div><b>3</b><span><strong>Recommended on hand</strong><small>Weekly baseline × target days of cover, increased by the safety-stock percentage, but never below the store minimum.</small></span></div><div><b>4</b><span><strong>Planning gap</strong><small>Recommended on hand − current accepted on-hand count. A positive value is a delivery planning signal; it is not an inventory correction.</small></span></div></div></div>
    <p className="store-outlook__note">Planning targets and seasonal inputs are stored in this browser for this workspace until organization-level configuration is connected. They remain deliberately separate from immutable scans.</p>
  </section>;
}
...
function ServicePlanQueue({ groups, totalGroups, pageSize, page, pageCount, onPageSizeChange, onPageChange, openLocation }: {
  ...
  return <div className="service-plan__queue-shell">
    ...
    {pageCount > 1 && <div className="service-plan__queue-pagination"><button type="button" className="secondary" disabled={page === 0} title={page === 0 ? "Already showing the first page" : "Show the previous locations"} aria-label={page === 0 ? "Previous locations unavailable: first page" : "Show previous locations"} onClick={() => onPageChange(Math.max(0, page - 1))}>Previous locations</button><strong>Page {page + 1} of {pageCount}</strong><button type="button" className="secondary" disabled={page >= pageCount - 1} title={page >= pageCount - 1 ? "Already showing the last page" : "Show the next locations"} aria-label={page >= pageCount - 1 ? "Next locations unavailable: last page" : "Show next locations"} onClick={() => onPageChange(Math.min(pageCount - 1, page + 1))}>Next locations</button></div>}
  </div>;
}
...
function ServicePlanPage({ data, openLocation }: { data: OperationsData; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void }) {
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (locationSearch.trim() && !`${row.location.name} ${locationTypeLabel(row.location)}`.toLocaleLowerCase().includes(locationSearch.trim().toLocaleLowerCase())) return false;
  ...
  const exportServicePlan = () => downloadCsv(`stacktrack-service-plan-${serviceDate}.csv`, [
    ...
    ...visibleRows.map((row) => [row.location.name, locationTypeLabel(row.location), row.goodsType, containerTypeLabel(row.containerType), row.full, row.empty, row.unknown, row.total, row.target.minimumOnHand, row.target.maximumOnHand, row.target.minimumEmpty, row.target.maximumFull, row.action, servicePriorityLabel(row.priority), row.pickupQty, row.deliveryQty, row.reason])
  ]);
        ...
        <section className="panel network-panel">
          ...
          {activeRoutes.length ? <div className="dashboard-route-list">{activeRoutes.slice(0, 4).map((route) => {
            ...
            return <button className="dashboard-route" key={route.key} onClick={() => setPage("locations")} aria-label={`View ${route.items.length} containers in transit after leaving ${route.origin?.name ?? "departure location not confirmed"}`}>
               <span className="dashboard-route__endpoint"><span className="dashboard-route__endpoint-icon">{route.origin ? <LocationTypeIcon location={route.origin} size={17} /> : <MapPin size={17} />}</span><span><strong title={route.origin?.name ?? "Origin pending"}>{route.origin?.name ?? "Origin pending"}</strong><small>{route.origin ? locationTypeLabel(route.origin) : "Origin not confirmed"}</small></span></span>
...
function DashboardInventoryMatrix({ data, setPage, openLocation, scoped = false }: { data: OperationsData; setPage: (page: Page) => void; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void; scoped?: boolean }) {
  ...
  const locationRows: InventoryMatrixRow[] = data.fixtures.locations
    ...
    .map((location) => ({ key: location.locationId, location, locationName: location.name, locationType: locationTypeLabel(location), records: recordsByLocation.get(location.locationId) ?? [] }));
  ...
  return <section className="panel inventory-matrix-panel" data-route-focus="inventory-matrix">
    ...
    <div className="inventory-matrix__filters"><label className="inventory-matrix__location-search"><span>Find a location</span><span className="inventory-matrix__location-search-input"><Search size={13} /><input type="search" value={locationSearch} onChange={(event) => setLocationSearch(event.target.value)} placeholder="Search location name" aria-label="Search inventory location" />{locationSearch && <button type="button" onClick={() => setLocationSearch("")} aria-label="Clear location search"><X size={13} /></button>}</span></label><label><span>Location scope</span><select value={locationScope} onChange={(event) => setLocationScope(event.target.value as InventoryLocationScope)}><option value="network">All locations</option><option value="donation_express">Donation Xpress</option><option value="store_backroom">Stores</option><option value="warehouse">Warehouses</option></select></label><AuditMultiSelect label="Container types" options={inventoryContainerTypes.map((value) => ({ value, label: containerTypeLabel(value) }))} selected={selectedContainerTypes} onToggle={(value) => setSelectedContainerTypes((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value])} onClear={() => setSelectedContainerTypes([])} emptyLabel="All container types" /><div className="inventory-matrix__filter-summary"><strong>{scopeLabel}</strong><span>{typeLabel}</span><small>{filteredLocationRows.length} locations · {records.length} containers included</small></div><button type="button" className="inventory-matrix__clear" onClick={() => { clearInventoryFilters(); setLocationSearch(""); }} disabled={locationScope === "network" && selectedContainerTypes.length === 0 && !locationSearch}>Clear filters</button></div>
      <div className="inventory-matrix__summary"><div><span><ContainerIcon size={15} />Tracked containers</span><strong>{records.length}</strong><small>{scopeLabel} · {typeLabel}</small></div><div><span><MapPin size={15} />At operating locations</span><strong>{physicalCount}</strong><small>Confirmed physical placement</small></div><div><span><Truck size={15} />In transit</span><strong>{transitRow?.records.length ?? 0}</strong><small>Leaving a confirmed location · next scan records arrival</small></div><div><span><CircleHelp size={15} />Unknown / unassigned</span><strong>{unknownRow?.records.length ?? 0}</strong><small>{unknownRow?.records.length ? "Needs location follow-up" : "No current location gaps"}</small></div></div>
     <div className="table-wrap inventory-matrix__table-wrap"><table className="inventory-matrix"><thead><tr><th>Location</th>{columns.map((column) => <th key={column.value}>{column.label}</th>)}<th>Total</th></tr></thead><tbody>{visibleLocationRows.map((row) => <tr key={row.key}><th scope="row"><button type="button" className="inventory-matrix__location" onClick={() => openRow(row)}><span className={`inventory-matrix__location-icon ${row.isUnknown ? "inventory-matrix__location-icon--unknown" : row.location ? `inventory-matrix__location-icon--${row.location.type}` : "inventory-matrix__location-icon--unknown"}`}>{row.location && !row.isUnknown ? <LocationTypeIcon location={row.location} size={15} /> : <CircleHelp size={15} />}</span><span><strong>{row.locationName}</strong><small>{row.locationType}{row.location?.isActive === false ? " · Inactive" : ""}</small></span><ChevronRight size={13} /></button></th>{columns.map((column) => { const items = recordsForColumn(row, column.value); return <td key={column.value}>{items.length ? <button type="button" className="inventory-matrix__cell" onClick={() => openRow(row, mode === "container" ? { containerType: column.value as Container["type"], bucket: "current" } : { goodsType: column.value, bucket: "current" })} title={`View ${items.length} ${column.label.toLowerCase()} at ${row.locationName}. ${statusSummary(items)}.`} aria-label={`View ${items.length} ${column.label.toLowerCase()} at ${row.locationName}`}><strong>{items.length}</strong><small>{statusSummary(items)}</small><em>{cellDetail(items)}</em></button> : <span className="inventory-matrix__empty">—</span>}</td>; })}<td><button type="button" className="inventory-matrix__cell inventory-matrix__cell--total" onClick={() => openRow(row, { bucket: "current" })} title={`View all ${row.records.length} containers at ${row.locationName}.`} aria-label={`View all ${row.records.length} containers at ${row.locationName}`}><strong>{row.records.length}</strong><small>{statusSummary(row.records) || "No current inventory"}</small></button></td></tr>)}</tbody><tfoot><tr><th>Filtered total</th>{columns.map((column) => <td key={column.value}><strong>{records.filter((record) => mode === "container" ? record.container.type === column.value : record.goodsType === column.value).length}</strong></td>)}<td><strong>{records.length}</strong></td></tr></tfoot></table></div>
     <div className="inventory-matrix__pagination"><span>Showing {visibleLocationRows.length ? safeLocationPage * (locationPageSize === "all" ? 0 : locationPageSize) + 1 : 0}–{Math.min(filteredLocationRows.length, locationPageSize === "all" ? filteredLocationRows.length : (safeLocationPage + 1) * locationPageSize)} of {filteredLocationRows.length} locations</span><label><span>Rows</span><select value={locationPageSize} onChange={(event) => { const value = event.target.value === "all" ? "all" : Number(event.target.value) as InventoryLocationPageSize; setLocationPageSize(value); setLocationPage(0); }}><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option><option value="all">All</option></select></label><button type="button" className="secondary" disabled={safeLocationPage === 0} title={safeLocationPage === 0 ? "Already showing the first page" : "Show the previous locations"} aria-label={safeLocationPage === 0 ? "Previous inventory locations unavailable: first page" : "Show previous inventory locations"} onClick={() => setLocationPage((current) => Math.max(0, current - 1))}>Previous</button><strong>Page {safeLocationPage + 1} of {locationPageCount}</strong><button type="button" className="secondary" disabled={safeLocationPage >= locationPageCount - 1} title={safeLocationPage >= locationPageCount - 1 ? "Already showing the last page" : "Show the next locations"} aria-label={safeLocationPage >= locationPageCount - 1 ? "Next inventory locations unavailable: last page" : "Show next inventory locations"} onClick={() => setLocationPage((current) => Math.min(locationPageCount - 1, current + 1))}>Next</button></div>
...
function getContainerRouteContext(containerId: string, data: OperationsData): ContainerRouteContext {
  ...
  // A local scanner can reliably report where a container left, but it cannot
  // know which truck or receiving site will take it.  The destination becomes
  // authoritative only when a later receiving scan is recorded.
  const departures: { event: StoredEvent; originId: string | null }[] = [];
  ...
  const segments = departures.map((departure, index): ContainerRouteSegment => {
    ...
    const receipt = events.find((event) => {
      ...
      // Do not use a legacy destinationLocationId here.  It was a planned
      // value supplied by an older client, not a confirmed receiving scan.
      return true;
    });
    return {
      ...
      destination: receipt ? locationFor(receipt.locationId) : null,
...
function locationLaneSummary(route: ContainerRouteContext, tone: "here" | "arriving" | "leaving"): string {
  if (tone === "leaving" && route.inTransit) return humanRouteSummary(route);
  if (tone === "arriving" && route.destination) {
    return `Received at ${route.destination.name}${route.origin ? ` from ${route.origin.name}` : ""}`;
  }
  return humanRouteSummary(route);
}
...
function ContainerImportPanel({ data, session, refresh, onClose }: { data: OperationsData; session: AdminSession; refresh: () => Promise<void>; onClose: () => void }) {
  const [fileName, setFileName] = useState("");
  const [rows, setRows] = useState<ContainerCsvRecord[]>([]);
  const [parseErrors, setParseErrors] = useState<ContainerImportValidationError[]>([]);
  const [busy, setBusy] = useState(false);
  const [notice, setNotice] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const allowedTypes = data.fixtures.containerTypes?.length
    ? data.fixtures.containerTypes
    : Array.from(new Set(data.fixtures.containers.map((container) => container.type))).sort();
  const existingLabels = data.fixtures.containers.map((container) => container.label);
  const selectFile = async (file: File | undefined) => {
    ...
    setError(null);
    try {
      const parsed = parseContainerCsv(await file.text());
      const validation = validateContainerRows(parsed.rows, allowedTypes, existingLabels);
      setRows(parsed.rows);
      setParseErrors([...parsed.errors, ...validation]);
    } catch (caught) {
      setRows([]);
      setParseErrors([{ line: 1, message: caught instanceof Error ? caught.message : "The CSV could not be read." }]);
    }
  };
  const submit = async () => {
    if (!rows.length || parseErrors.length || busy) return;
    setBusy(true);
    setError(null);
    setNotice(null);
    try {
      const result = await importContainers(session, rows.map((row) => ({ label: row.label, type: row.type })));
      await refresh();
      setNotice(`${result.importedCount.toLocaleString()} container${result.importedCount === 1 ? "" : "s"} imported. Existing containers were not changed.`);
      setRows([]);
      setFileName("");
    } catch (caught) {
      if (caught instanceof ApiRequestError) {
        const details = caught.details as { rowErrors?: Array<{ row: number; message: string }> } | undefined;
        if (details?.rowErrors?.length) setParseErrors(details.rowErrors.map((item) => ({ line: item.row, message: item.message })));
      }
      setError(caught instanceof Error ? caught.message : "The import was rejected. Nothing was changed.");
    } finally {
      setBusy(false);
    }
  };
  return <section className="panel container-import-panel" aria-label="Import containers">
    <div className="container-import-panel__header"><div><span className="eyebrow">Controlled bulk import</span><h2>Import containers from CSV</h2><p>StackTrack validates every row before writing anything. If one row is wrong, the whole file is rejected and no existing container is overwritten.</p></div><button className="icon-button" type="button" aria-label="Close container import" onClick={onClose}><X size={16} /></button></div>
    <div className="container-import-panel__rules"><strong>File requirements</strong><span>Exactly two columns, in this order: <code>label,container_type</code></span><span>Use an active type listed below: {allowedTypes.length ? allowedTypes.join(", ") : "none configured"}</span><span>Labels must be unique, new, and 120 characters or fewer. Do not include IDs, locations, status, or extra columns.</span><button className="secondary" type="button" onClick={() => downloadCsv("stacktrack-container-import-template.csv", [["label", "container_type"], ["B4001", allowedTypes[0] ?? "bin"]])}><Download size={14} /> Download template</button></div>
    <label className="container-import-file"><span>Choose CSV file</span><input type="file" accept=".csv,text/csv" onChange={(event) => void selectFile(event.target.files?.[0])} disabled={busy} /><small>{fileName || "No file selected"}</small></label>
    {parseErrors.length > 0 && <div className="container-import-errors" role="alert"><strong>{parseErrors.length} issue{parseErrors.length === 1 ? "" : "s"} found — nothing will be imported.</strong><ul>{parseErrors.sort((left, right) => left.line - right.line).map((item, index) => <li key={`${item.line}:${index}`}><b>Row {item.line}:</b> {item.message}</li>)}</ul></div>}
    {rows.length > 0 && !parseErrors.length && <div className="container-import-ready"><CheckCircle2 size={17} /><span><strong>{rows.length.toLocaleString()} rows ready.</strong> All local checks passed; the server will repeat them inside one transaction.</span></div>}
    {notice && <div className="container-import-success" role="status"><CheckCircle2 size={17} />{notice}</div>}
    {error && <div className="container-import-errors" role="alert"><strong>Import rejected — nothing was changed.</strong><span>{error}</span></div>}
    <div className="container-import-panel__footer"><span>Current active types: {allowedTypes.length ? allowedTypes.join(", ") : "none"}</span><div><button className="secondary" type="button" onClick={onClose}>Close</button><button className="primary" type="button" disabled={busy || !rows.length || parseErrors.length > 0} onClick={() => void submit()}>{busy ? "Importing…" : rows.length ? `Import ${rows.length.toLocaleString()} containers` : "Import containers"}</button></div></div>
  </section>;
}
...
function locationTypeLabel(value: Location["type"] | Location) {
  if (typeof value !== "string" && value.typeName) return value.typeName;
  const type = typeof value === "string" ? value : value.type;
  return type === "donation_express" ? "Donation Xpress" : type === "warehouse" ? "Warehouse" : type === "in_transit" ? "In transit" : type === "other" ? "Other" : "Store";
}
...
/**
 * A departure does not carry a destination.  A location can be called an
 * arrival only when the latest accepted event is a batch-in at that site.
 * Keeping this derived from the latest event prevents open departures from
 * appearing in every possible destination column.
 */
function latestLocationEventForContainer(containerId: string, data: OperationsData): StoredEvent | null {
  return data.events
    .filter((event) => event.containerId === containerId)
    .filter((event) => event.eventType === "batch_in" || event.eventType === "batch_out")
    .sort((left, right) => eventSortTimestamp(right) - eventSortTimestamp(left) || right.eventId.localeCompare(left.eventId))[0] ?? null;
}

function wasReceivedAtLocation(projection: Projection, locationId: string, data: OperationsData): boolean {
  const latest = latestLocationEventForContainer(projection.containerId, data);
  return latest?.eventType === "batch_in" && latest.locationId === locationId;
}
...
function LocationNetworkOverview({ metrics, movingCount, movingReviewCount, routeRecords, onSelect, onOpen }: { metrics: LocationMetric[]; movingCount: number; movingReviewCount: number; routeRecords: RouteRecord[]; onSelect: (locationId: string) => void; onOpen: (record: RouteRecord) => void }) {
  ...
  const renderLocationNode = (metric: LocationMetric) => <button className="location-flow-node" key={metric.location.locationId} onClick={() => onSelect(metric.location.locationId)}>
    ...
    <span className="location-flow-node__body"><strong>{metric.location.name}</strong><small>{locationTypeLabel(metric.location)} - {metric.current.length} here - {metric.arriving.length} received here</small></span>
...
function LocationRouteMatrix({ routeRecords, onSelect }: { routeRecords: RouteRecord[]; onSelect: (locationId: string) => void }) {
  ...
  return <section className="location-option panel"><div className="location-option__header"><div><span className="eyebrow">Location view option 2 · route matrix</span><h2>Recorded handoff workload</h2><p>Use this when operations leadership needs to compare confirmed handoffs, find reroutes, or see which warehouse departures are still open. Each row is an independent checkpoint; rows do not imply a single chain.</p></div><span className="location-option__icon"><Waypoints size={19} /></span></div><div className="location-option__summary"><span><b>{rows.length}</b> recorded routes</span><span><b>{rows.reduce((total, row) => total + row.active, 0)}</b> active</span><span><b>{rows.reduce((total, row) => total + row.received, 0)}</b> completed handoffs</span><span className={unresolved ? "location-option__summary--warn" : ""}><b>{unresolved}</b> open departures</span></div>{rows.length ? <div className="route-matrix"><div className="route-matrix__head"><span>Origin</span><span>Receiving site</span><span>Containers / status</span><span>Last departure</span><span /></div>{rows.map((row) => <button className="route-matrix__row" key={`${row.origin?.locationId ?? "unknown"}:${row.destination?.locationId ?? "unknown"}`} onClick={() => onSelect(row.origin?.locationId ?? row.destination?.locationId ?? "")}><span><strong>{row.origin?.name ?? "Origin not recorded"}</strong><small>{row.origin ? locationTypeLabel(row.origin) : "Needs review"}</small></span><span className="route-matrix__destination"><ArrowRight size={14} /><strong>{row.destination?.name ?? "Receiving site not confirmed"}</strong></span><span><strong>{row.containers.length}</strong><small>{row.active ? `${row.active} active` : "No active"}{row.received ? ` · ${row.received} completed` : ""}{row.superseded ? ` · ${row.superseded} rerouted` : ""}</small></span><span>{row.lastDeparture ? relativeTime(row.lastDeparture) : "—"}{row.review > 0 && <Pill tone="warn">{row.review} review</Pill>}</span><ChevronRight size={15} /></button>)}</div> : <EmptyState>No route handoffs have been recorded yet.</EmptyState>}</section>;
}
...
function LocationWorkspacePage({ data, locationId, locationFilter, openLocation, openDetail, setPage, refresh, session }: { data: OperationsData; locationId: string; locationFilter?: LocationInventoryFilter; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void; openDetail: OpenDetail; setPage: (page: Page) => void; refresh: () => Promise<void>; session: AdminSession | null }) {
  ...
  const arriving = current.filter((projection) => wasReceivedAtLocation(projection, location.locationId, data));
  ...
  const recordsForBucket = (bucket: LocationInventoryBucket) => inventoryRecords.filter((record) => {
    ...
    const isArriving = Boolean(record.projection && wasReceivedAtLocation(record.projection, location.locationId, data));
...
function LocationsPage({ data, focusedLocationId, focusedLocationFilter, openContainers, openLocation, openDetail, setPage, refresh, session }: { data: OperationsData; focusedLocationId?: string; focusedLocationFilter?: LocationInventoryFilter; openContainers: (movement?: ContainerMovement) => void; openLocation: (locationId: string, filter?: LocationInventoryFilter) => void; openDetail: OpenDetail; setPage: (page: Page) => void; refresh: () => Promise<void>; session: AdminSession | null }) {
  ...
  const [locationPageSize, setLocationPageSize] = useState<12 | 24 | 50 | "all">(24);
  const [locationPage, setLocationPage] = useState(0);
  useEffect(() => {
    setLocationPage(0);
  ...
  const matchingMetrics = locationMetrics
    ...
    .filter((metric) => !locationSearch || `${metric.location.name} ${locationTypeLabel(metric.location)}`.toLowerCase().includes(locationSearch))
    .sort((left, right) => {
  ...
  const locationPageCount = locationPageSize === "all" ? 1 : Math.max(1, Math.ceil(matchingMetrics.length / locationPageSize));
  const safeLocationPage = Math.min(locationPage, locationPageCount - 1);
  ...
  const exportFilteredLocations = () => downloadCsv("stacktrack-locations-filtered.csv", [
    ...
    ...matchingMetrics.map((metric) => [
      ...
      locationTypeLabel(metric.location),
    ...
    {false && <section className="location-selector panel">
     ...
     {false && <section className="location-workspace panel">
      ...
      <div className="workflow-lanes">
        ...
        <LocationWorkflowLane title="Recently received" subtitle="Latest accepted arrival scans recorded at this location" tone="arriving" items={arriving} data={data} onOpen={openContainer} />
...
const fallbackLocationTypes: LocationType[] = [
...
function locationTypeIconRecord(type: LocationType): Location {
  return { locationId: `type-${type.typeKey}`, name: type.name, type: type.category, typeKey: type.typeKey, typeName: type.name, iconKey: type.iconKey, isActive: true };
}

function LocationTypeAdministration({ types, session, refresh, canManage, onNotice, onError }: { types: LocationType[]; session: AdminSession | null; refresh: () => Promise<void>; canManage: boolean; onNotice: (message: string) => void; onError: (message: string) => void }) {
  const [createOpen, setCreateOpen] = useState(false);
  const [name, setName] = useState("");
  const [category, setCategory] = useState<Exclude<LocationTypeCategory, "in_transit">>("other");
  ...
  if (!types.length) return <section className="location-type-admin"><div className="location-type-admin__header"><div><span className="eyebrow">Location types</span><strong>No location types are available yet.</strong><small>Apply the location catalog database migration before adding locations.</small></div></div></section>;
  const saveType = async (type: LocationType) => {
    ...
    try {
      ...
      onError(caught instanceof Error ? caught.message : "Location type could not be updated.");
    } finally {
      setBusyKey(null);
    }
  };
  const createType = async (event: React.FormEvent) => {
    ...
    setBusyKey("new");
    try {
      await createLocationType(session, { name: name.trim(), category, iconKey });
      await refresh();
      setName("");
      setCategory("other");
      setIconKey("map-pin");
      setCreateOpen(false);
      onNotice("Location type added with its default map-pin icon. You can change the icon below at any time.");
    } catch (caught) {
      onError(caught instanceof Error ? caught.message : "Location type could not be added.");
    } finally {
      setBusyKey(null);
    }
  };
  return <section className="location-type-admin">
    <div className="location-type-admin__header"><div><span className="eyebrow">Location types</span><strong>Set the names and icons used across the directory.</strong><small>Changing a type name or icon updates future display everywhere without rewriting historical scans. “In transit” stays a protected system type.</small></div>{canManage && <button className="secondary" type="button" onClick={() => setCreateOpen((value) => !value)}>{createOpen ? <X size={14} /> : <Plus size={14} />}{createOpen ? "Close" : "Add type"}</button>}</div>
    {createOpen && canManage && <form className="location-type-create" onSubmit={(event) => void createType(event)}><label><span>New type name</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Example: Outlet" maxLength={80} autoFocus /></label><label><span>Operational category</span><select value={category} onChange={(event) => setCategory(event.target.value as Exclude<LocationTypeCategory, "in_transit">)}><option value="other">Other operating site</option><option value="store_backroom">Store-like</option><option value="donation_express">Donation Xpress-like</option><option value="warehouse">Warehouse-like</option></select></label><label><span>Default icon</span><select value={iconKey} onChange={(event) => setIconKey(event.target.value)}><option value="map-pin">Map pin</option><option value="building-2">Building</option><option value="boxes">Boxes</option><option value="store">Store</option><option value="warehouse">Warehouse</option><option value="hand-heart">Donation Xpress</option><option value="package-check">Package</option></select></label><button className="primary" type="submit" disabled={busyKey === "new" || name.trim().length < 2}>{busyKey === "new" ? "Adding…" : "Add location type"}</button></form>}
    <div className="location-type-list">{types.map((type) => { const draft = drafts[type.typeKey] ?? { name: type.name, iconKey: type.iconKey }; return <div className="location-type-row" key={type.typeKey}><span className="location-type-row__icon"><LocationTypeIcon location={locationTypeIconRecord({ ...type, ...draft })} size={17} /></span><div className="location-type-row__identity"><strong>{type.name}</strong><small>{type.isSystem ? "System type" : `Custom type · ${type.category === "other" ? "Other" : locationTypeLabel(type.category)}`}</small></div>{canManage ? <><label className="location-type-row__field"><span className="sr-only">Name for {type.name}</span><input value={draft.name} onChange={(event) => setDrafts((current) => ({ ...current, [type.typeKey]: { ...draft, name: event.target.value } }))} maxLength={80} /></label><label className="location-type-row__field"><span className="sr-only">Icon for {type.name}</span><select value={draft.iconKey} onChange={(event) => setDrafts((current) => ({ ...current, [type.typeKey]: { ...draft, iconKey: event.target.value } }))}><option value="map-pin">Map pin</option><option value="building-2">Building</option><option value="boxes">Boxes</option><option value="store">Store</option><option value="warehouse">Warehouse</option><option value="hand-heart">Donation Xpress</option><option value="truck">Truck</option><option value="package-check">Package</option></select></label><button className="secondary" type="button" disabled={busyKey !== null || draft.name.trim().length < 2} onClick={() => void saveType(type)}>Save</button></> : <span className="location-type-row__readonly">{type.name}</span>}</div>; })}</div>
  </section>;
}
...
function LocationAdministrationPanel({
  ...
  const [locationType, setLocationType] = useState<string>("store_backroom");
  ...
  const locationTypes = data.fixtures.locationTypes?.length ? data.fixtures.locationTypes : fallbackLocationTypes;
  ...
  return <section className="location-admin-panel panel">
    ...
    {createOpen && canCreate && <form className="location-create-form" onSubmit={(event) => void submitCreate(event)}>
      ...
      <label><span>Location type</span><select value={locationType} onChange={(event) => setLocationType(event.target.value)} disabled={busy}>{locationTypes.filter((type) => type.category !== "in_transit" && type.isActive).map((type) => <option value={type.typeKey} key={type.typeKey}>{type.name}</option>)}</select></label>
    ...
    <LocationTypeAdministration types={locationTypes} session={session} refresh={refresh} canManage={Boolean(canCreate)} onNotice={setNotice} onError={setError} />
    <LocationDirectoryEditor locations={activeLocations} types={locationTypes} session={session} refresh={refresh} canManage={Boolean(canCreate)} onNotice={setNotice} onError={setError} />
...
function LocationWorkflowLane({ title, subtitle, tone, items, data, onOpen }: { title: string; subtitle: string; tone: "here" | "arriving" | "leaving"; items: Projection[]; data: OperationsData; onOpen: (projection: Projection) => void }) {
  ...
  return <section className={`workflow-lane workflow-lane--${tone}`}>
    ...
    <div className="workflow-lane__items">{visible.length ? visible.map((projection) => {
      ...
      const route = getContainerRouteContext(projection.containerId, data);
      const routeLabel = locationLaneSummary(route, tone);
...
function ActivityPage({ data, query, openDetail, setPage, session }: { data: OperationsData; query: string; openDetail: OpenDetail; setPage: (page: Page) => void; session: AdminSession }) {
  ...
  const events = data.events.filter((event) => {
    ...
    const relatedLocationIds = [event.locationId, payloadLocationId(event, "sourceLocationId")].filter((value): value is string => Boolean(value));
    const searchable = [
      containerName(event.containerId),
  ...
  return <><RoleScopeNotice principal={session.principal} pageLabel="Activity" /><section className="panel activity-page">
    ...
    <div className="activity-filter-panel">
      ...
      <div className="activity-filters activity-filters--expanded">
        <AuditMultiSelect stacked label="Actions" options={actionOptions} selected={eventFilters} onToggle={(value) => setEventFilters((current) => current.includes(value as StoredEvent["eventType"]) ? current.filter((item) => item !== value) : [...current, value as StoredEvent["eventType"]])} onClear={() => setEventFilters([])} emptyLabel="All actions" />
        <AuditMultiSelect stacked label="Locations" options={locationOptions} selected={locationFilters} onToggle={(value) => setLocationFilters((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value])} onClear={() => setLocationFilters([])} emptyLabel="All locations" />
        <AuditMultiSelect stacked label="Scanners" options={deviceOptions} selected={deviceFilters} onToggle={(value) => setDeviceFilters((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value])} onClear={() => setDeviceFilters([])} emptyLabel="All scanners" />
        <label><span className="activity-filter-label">Quick time window</span><select value={windowFilter} onChange={(event) => setWindowFilter(event.target.value as ActivityWindow)}><option value="all">All available</option><option value="today">Last 24 hours</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select></label>
        <label><span className="activity-filter-label">From date</span><input type="date" value={fromDate} onChange={(event) => setFromDate(event.target.value)} /></label>
        <label><span className="activity-filter-label">To date</span><input type="date" value={toDate} onChange={(event) => setToDate(event.target.value)} /></label>
        <label><span className="activity-filter-label">From time</span><input type="time" value={fromTime} onChange={(event) => setFromTime(event.target.value)} /></label>
        <label><span className="activity-filter-label">To time</span><input type="time" value={toTime} onChange={(event) => setToTime(event.target.value)} /></label>
      </div>
    ...
    {filtersInvalid ? <EmptyState>Adjust the time range to see scanner observations.</EmptyState> : events.length ? <div className="activity-results"><div className="activity-results-toolbar"><span>Showing {safePageIndex * (pageSize === "all" ? 0 : pageSize) + 1}–{Math.min(events.length, pageSize === "all" ? events.length : (safePageIndex + 1) * pageSize)} of {events.length} observations</span><label><span>Rows</span><select value={pageSize} onChange={(event) => { const next = event.target.value === "all" ? "all" : Number(event.target.value) as ActivityPageSize; setPageSize(next); setPageIndex(0); }} aria-label="Observations per page"><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option><option value={200}>200</option><option value="all">All</option></select></label></div><div className="timeline">{visibleEvents.map((event, index) => {
       ...
       const routeText = eventSegment ? humanSegmentSummary(eventSegment) : null;
...
function ReportsPage({ data, openDetail }: { data: OperationsData; openDetail: OpenDetail }) {
  ...
  const projectionMatches = (projection: Projection) => {
    ...
    const relatedLocationIds = [projection.locationId, ...relatedEvents.flatMap((event) => [event.locationId, payloadLocationId(event, "sourceLocationId")])].filter((value): value is string => Boolean(value));
  ...
  const transitRows = filteredProjections
    ...
    .map((projection) => {
      ...
      const receivingLocationId = route.activeSegment?.destination?.locationId ?? null;
      const ageHours = outbound ? Math.max(0, (Date.now() - Date.parse(outbound.effectiveAt)) / 3_600_000) : null;
      return { projection, outbound, originId, receivingLocationId, ageHours };
    })
  ...
  const reports = [
    ...
    { id: "transit", icon: Truck, title: "Transit aging", text: "Containers still in motion, their confirmed departure origin, and how long the receiving scan has been outstanding.", count: transitRows.length, tag: "Ready" },
    { id: "routes", icon: GitBranch, title: "Multi-hop route ledger", text: "Every departure and confirmed receiving handoff in sequence, including active transfers and superseded departures.", count: routeRows.length, tag: "Ready" },
...
const emptyAuditFilters: AuditDraft = { search: "", locationIds: [], deviceIds: [], actionPrefixes: [], targetTypes: [], from: "", to: "" };
...
function auditRequestFilters(filters: AuditDraft) {
  const { locationIds, deviceIds, ...rest } = filters;
  return {
    ...rest,
    ...(locationIds.length ? { selectedLocationIds: locationIds } : {}),
    ...(deviceIds.length ? { selectedDeviceIds: deviceIds } : {}),
...
function AuditTrailPage({ data, session, openDetail }: { data: OperationsData; session: AdminSession; openDetail: OpenDetail }) {
  ...
  const updateFilter = (field: "search" | "from" | "to", value: string) => setDraft((current) => ({ ...current, [field]: value }));
  useEffect(() => {
  ...
  const toggleAuditFilter = (field: "actionPrefixes" | "targetTypes", value: string) => setDraft((current) => ({ ...current, [field]: current[field].includes(value) ? current[field].filter((item) => item !== value) : [...current[field], value] }));
...
function ExceptionsPage({ data, openDetail, session, refresh }: { data: OperationsData; openDetail: OpenDetail; session: AdminSession; refresh: () => Promise<void> }) {
  ...
  const scoped = isScopedPrincipal(session.principal);
  return <section className="panel exceptions-panel">
    {scoped && <div className="review-scope-note"><MapPin size={19} /><div><strong>Assigned-location review</strong><p>You can inspect preserved evidence for your assigned locations. Corporate operations administrators handle review decisions and official history changes.</p></div><Pill tone="blue">View only</Pill></div>}
    <div className="review-summary"><span><b>{activeCases.length}</b> active cases</span><span><b>{data.reviewCases.length - activeCases.length}</b> completed history</span><span>{scoped ? "Corporate review decisions are handled centrally." : "Organization Owners can resolve material cases."}</span></div>
