...
function LocationTargetWorkspace({ stores, rows, targets, containerOptions, editorStore, setEditorStore, editorGoods, setEditorGoods, editorContainer, setEditorContainer, editorMinimum, setEditorMinimum, editorMaximum, setEditorMaximum, onSave, onReset }: { stores: Location[]; rows: StoreOutlookRow[]; targets: Record<string, StoreOutlookTarget>; containerOptions: readonly ServiceContainerType[]; editorStore: string; setEditorStore: (value: string) => void; editorGoods: string; setEditorGoods: (value: string) => void; editorContainer: ServiceContainerType; setEditorContainer: (value: ServiceContainerType) => void; editorMinimum: string; setEditorMinimum: (value: string) => void; editorMaximum: string; setEditorMaximum: (value: string) => void; onSave: (event: React.FormEvent) => void; onReset: () => void }) {
  const [attentionOnly, setAttentionOnly] = useState(false);
  const selectedStore = stores.find((store) => store.locationId === editorStore) ?? stores[0];
  if (!selectedStore) return <section className="store-outlook__target-editor store-outlook__target-editor--new"><EmptyState>No store locations are available for target setup.</EmptyState></section>;
  ...
  const selectedRow = locationRows.find((row) => row.goodsType === editorGoods && row.containerType === editorContainer);
  ...
  return <section className="store-outlook__target-editor store-outlook__target-editor--new" aria-labelledby="location-target-setup-title">
    <div className="store-outlook__target-header"><div><span className="eyebrow">Location target setup</span><h3 id="location-target-setup-title">Choose a location, then tune its item targets</h3><p>Start with one store at a time. The location summary stays compact, while the detailed goods and container settings appear only for the selected store.</p></div><label className="store-outlook__target-location"><span>Location</span><select value={selectedStore.locationId} onChange={(event) => setEditorStore(event.target.value)}>{stores.map((store) => <option key={store.locationId} value={store.locationId}>{store.name}</option>)}</select></label></div>
    ...
    <div className="store-outlook__target-details"><div className="store-outlook__target-details-heading"><div><span className="eyebrow">Item targets at {selectedStore.name}</span><strong>Review the range before editing</strong><p>Each row represents one goods category and container type. Select Edit target to change the minimum and maximum for that combination.</p></div><label className="store-outlook__target-check"><input type="checkbox" checked={attentionOnly} onChange={(event) => setAttentionOnly(event.target.checked)} /> <span>Show planning signals only</span></label></div><div className="store-outlook__target-table-wrap table-wrap"><table className="store-outlook__target-table"><thead><tr><th>Goods category</th><th>Container</th><th>On hand</th><th>Target range</th><th>Status</th><th></th></tr></thead><tbody>{detailRows.length ? detailRows.map((row) => { const status = statusFor(row); const isSelected = row.goodsType === editorGoods && row.containerType === editorContainer; return <tr className={isSelected ? "store-outlook__target-row--selected" : ""} key={storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)}><th><strong>{row.goodsType}</strong><small>{row.holidayUplift ? `${row.holidayUplift > 0 ? "+" : ""}${row.holidayUplift}% seasonal adjustment` : "No seasonal adjustment"}</small></th><td>{containerTypeLabel(row.containerType)}</td><td><strong>{row.current}</strong><small>{Math.round(row.weeklyBaseline * 10) / 10}/week baseline</small></td><td>{row.target.minimumOnHand}–{row.target.maximumOnHand}<small>{targets[storeOutlookTargetKey(row.store.locationId, row.goodsType, row.containerType)] ? "Custom range" : "Default range"}</small></td><td><Pill tone={status.tone}>{status.label}</Pill><small>{status.detail}</small></td><td><button type="button" className="secondary" onClick={() => { setEditorGoods(row.goodsType); setEditorContainer(row.containerType); }}>{isSelected ? "Editing" : "Edit target"}</button></td></tr>; }) : <tr><td colSpan={6}><EmptyState>No item types are below the selected filter.</EmptyState></td></tr>}</tbody></table></div></div>
    <form className="store-outlook__target-form" onSubmit={onSave}><div className="store-outlook__target-form-heading"><div><span className="eyebrow">Edit one target</span><strong>{selectedStore.name} · {selectedRow?.goodsType ?? editorGoods} · {selectedRow ? containerTypeLabel(selectedRow.containerType) : containerTypeLabel(editorContainer)}</strong><p>These values affect planning recommendations only. They do not change scans or the official container record.</p></div><span className="store-outlook__target-form-hint">{selectedRow ? `${selectedRow.current} currently on hand` : "Choose a valid item type above"}</span></div><div className="store-outlook__target-form-fields"><label><span>Goods category</span><select value={editorGoods} onChange={(event) => setEditorGoods(event.target.value)}>{Array.from(new Set(rows.filter((row) => row.store.locationId === selectedStore.locationId).map((row) => row.goodsType))).map((goodsType) => <option value={goodsType} key={goodsType}>{goodsType}</option>)}</select></label><label><span>Container type</span><select value={editorContainer} onChange={(event) => setEditorContainer(event.target.value as ServiceContainerType)}>{containerOptions.map((containerType) => <option value={containerType} key={containerType}>{containerTypeLabel(containerType)}</option>)}</select></label><label><span>Minimum on hand</span><input type="number" min="0" max="999" value={editorMinimum} onChange={(event) => setEditorMinimum(event.target.value)} /></label><label><span>Maximum on hand</span><input type="number" min="0" max="999" value={editorMaximum} onChange={(event) => setEditorMaximum(event.target.value)} /></label><div className="store-outlook__target-actions"><button type="button" className="secondary" onClick={onReset}>Use default range</button><button type="submit" className="primary"><Target size={15} /> Save target</button></div></div></form>
  </section>;
}
