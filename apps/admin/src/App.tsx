...
const locationIconOptions = [
  ...
  { value: "hand-heart", label: "Donation Xpress" },
...
function LocationTypeAdministration({ types, session, refresh, canManage, onNotice, onError }: { types: LocationType[]; session: AdminSession | null; refresh: () => Promise<void>; canManage: boolean; onNotice: (message: string) => void; onError: (message: string) => void }) {
  ...
  const createType = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setIconKey("map-pin");
      setCreateOpen(false);
      const iconLabel = locationIconOptions.find((option) => option.value === iconKey)?.label ?? "selected";
      onNotice(`Location type added with the ${iconLabel.toLowerCase()} icon. You can change the icon below at any time.`);
    } catch (caught) {
