...
describe("location administration routes", () => {
  it("delegates location name/type edits and location-type catalog changes", async () => {
    ...
    app = await createApp({
      ...
      locationAdministration: { update, createType, updateType, deleteType } as unknown as LocationAdministration
    });
    ...
    expect(deleteType).toHaveBeenCalledWith(tenantId, { userId: owner.userId }, "outlet");
  });
  ...
  it("does not let a read-only reviewer change names or location types", async () => {
    ...
    app = await createApp({
      ...
      locationAdministration: { update, createType, deleteType } as unknown as LocationAdministration
    });
    ...
    expect(deleteTypeResponse.statusCode).toBe(403);
