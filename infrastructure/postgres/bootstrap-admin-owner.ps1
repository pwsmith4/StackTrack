...
$postgresBin = @("C:\Program Files\PostgreSQL\18\bin", "C:\Program Files\PostgreSQL\16\bin") | Where-Object { Test-Path (Join-Path $_ "psql.exe") } | Select-Object -First 1
...
if ($username -notmatch '^[a-z0-9._-]{3,64}$') { throw "Username must use 3–64 lowercase letters, numbers, periods, underscores, or hyphens." }
...
"@ | & $psql --host=$ServerName --port=5432 --username=$AdminLogin --dbname=stacktrack --set=ON_ERROR_STOP=1 --set="tenant_id=$TenantId" --set="username=$username" --set="display_name=$DisplayName" --set="password_hash=$passwordHash"
